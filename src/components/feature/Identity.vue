      ...
      <v-card>
        ...
        <v-data-table :headers="headers" :items="observedDatas.data" :pagination.sync="pagination" :total-items="observedDatas.size"
          ...
          <template v-slot:items="props">
            <tr>
              <td>{{ props.item.id }}</td>
              <td>{{ props.item.name.replace(/.(generic)/g, '') }}</td>
          ...
          <template v-slot:expand="props">
            <v-card flat style="background-color: #f7f7f7;">
              <v-card-title primary-title>
                <div>
                  <div class="headline">Identity Detail</div>
                </div>
              </v-card-title>
              <v-card-text>
                <div class="row">
                  ...
                  <div class="col-md-10 col-9 col-sm-8">: {{ props.item.name.replace(/.(generic)/g, '') }}</div>
                  ...
                  <div class="col-md-2 col-3 col-sm-4 font-weight-bold">&#9500; Identity Class</div>
                  <div class="col-md-10 col-9 col-sm-8">: {{ props.item.identity_class }}</div>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      params: {
        handler() {
          this.$store.dispatch(types.GET_ALL_IDENTITY, this.params)
        }
      }
    },
    computed: {
      observedDatas() {
        return this.$store.getters[types.ALL_IDENTITY]
      },
