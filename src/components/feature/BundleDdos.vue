          ...
          <template v-slot:items="props">
            <tr>
              ...
              <td style="width: 140px;">
                <button class="btn btn-primary" style="margin-right: 8px;" @click="props.expanded = !props.expanded">
                  <font-awesome-icon icon="expand" /></button>
                <button class="btn btn-danger">
                  <font-awesome-icon icon="eye" @click="changeBundle(props.item)" /></button>
              </td>
            </tr>
          </template>
              ...
              <v-card-text>
                <div class="row">
                  ...
                  <div class="col-md-10 col-9 col-sm-8">:
                    {{ props.item.objects[4].name.replace(/.(threat actor)/g, '') }}</div>

                  <div class="col-md-2 col-3 col-sm-4 font-weight-bold">&#9500; Target</div>
                  <div class="col-md-10 col-9 col-sm-8">: {{ props.item.objects[3].name.replace(/.(generic)/g, '') }}
                  </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      params: {
        handler() {
          this.$store.dispatch(types.GET_ALL_BUNDLE_DDOS, this.params)
        }
      }
    },
    computed: {
      bundles() {
        return this.$store.getters[types.ALL_BUNDLE_DDOS]
      },
      bundlesIsLoading() {
        return this.$store.getters[types.BUNDLE_DDOS_LOADING]
      },
      selectedBundle() {
        return this.$store.getters[types.BUNDLE_DDOS_SELECTED_BUNDLE]
      },
      selectedNode() {
        return this.$store.getters[types.BUNDLE_DDOS_SELECTED_NODE]
      },
      legend() {
        return this.$store.getters[types.BUNDLE_DDOS_LEGEND]
      }
    },
    methods: {
      changeBundle(data) {
        this.$store.dispatch(types.CHANGE_BUNDLE_DDOS_SELECTED_BUNDLE, data)
      }
    },
    mounted() {
      this.height = 700
      ...
      this.$store.dispatch(types.GET_ALL_BUNDLE_DDOS)
    }
  }
</script>
<style>
